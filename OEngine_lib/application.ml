...
module ImGuiLayer = struct
  ...
  let create glfw =
    assert (
      Imguiml.Bindings.debug_check_version_and_data_layout
        (Imguiml.Bindings.get_version ())
        (sizeof Imguiml.Types.Io.t)
        (sizeof Imguiml.Types.Style.t)
        (sizeof Imguiml.Types.Vec2.t)
        (sizeof Imguiml.Types.Vec4.t)
        (sizeof Imguiml.Types.DrawVert.t)
    ...
    let* _ =
      ...
      | Some w ->
          let ptr =
            ...
            |> Ctypes.coerce
                 (ptr Bindings.Types.Glfw.window)
                 (ptr Imguiml_impl.Glfw.window)
          in
          if Imguiml_impl.Glfw.init_for_opengl ptr true then
            ...
            error
              (Core.Error.OEngine
                 (Core.Error.Custom "Failed to init imgui with GLFW"))
      | None -> ok ()
    in
  ...
  let update app =
    ...
    let size =
      Imguiml.Bindings.ImVec2.float_ (float_of_int width) (float_of_int heigth)
    in
...
let make (module FirstLayer : Layer.APPLICATION_LAYER) =
  ...
  let window =
    Glfw.Window.make ~width:1920 ~height:1080 ~title:"GLFW Window" ()
  in
  let* glfw =

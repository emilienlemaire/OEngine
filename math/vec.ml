...
let zeros elt dim = Genarray.init (Base.kind elt) c_layout [| dim |] (fun _ -> Base.zero elt)

let ones elt dim = Genarray.init (Base.kind elt) c_layout [| dim |] (fun _ -> Base.one elt)

let shape arr = (Genarray.dims arr).(0)

let init elt dim f = genarray_of_array1 @@ Array1.init (Base.kind elt) c_layout dim f
...
let scalar_mul s arr =
  ...
  genarray_of_array1
  @@ Array1.init (Genarray.kind arr) c_layout n (fun i -> mul (Array1.unsafe_get arr1 i) s)

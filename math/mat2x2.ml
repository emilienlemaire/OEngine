...
let empty elt = Genarray.create (Base.kind elt) c_layout [| 2; 2 |]

let zeros elt = Genarray.init (Base.kind elt) c_layout [| 2; 2 |] (fun _ -> Base.zero elt)

let ones elt = Genarray.init (Base.kind elt) c_layout [| 2; 2 |] (fun _ -> Base.one elt)

let share _arr = (2, 2)

let init elt f = Genarray.init (Base.kind elt) c_layout [| 2; 2 |] (fun arr -> f arr.(0) arr.(1))

let scalar_mul s arr =
  let elt = Base.elt_of_kind @@ Genarray.kind arr in
  let mul = Base.mul elt in
  Genarray.init (Genarray.kind arr) c_layout [| 2; 2 |] (fun idx -> mul (Genarray.get arr idx) s)
...
let _mat_member_op : type a b. ((a, b) Base.elt -> a Base.binop) -> (a, b) t -> (a, b) t -> (a, b) t
 ...
 fun op m1 m2 ->
  if Genarray.kind m1 <> Genarray.kind m2 then
    ...
    let op_ = op elt in
    Genarray.init (Genarray.kind m1) c_layout [| 2; 2 |] (fun i ->
...
let mul : type a b. (a, b) mat_op =
 fun m1 m2 ->
  if Genarray.kind m1 <> Genarray.kind m2 then
    ...
    in
    Genarray.init (Base.kind elt) c_layout [| 2; 2 |] (fun arr ->

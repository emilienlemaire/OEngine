...
module Types = struct
  module Gl = struct
    ...
    let int_as_uint = view ~read:Unsigned.UInt.to_int ~write:Unsigned.UInt.of_int uint

    let ba_as_charp =
      view
        ~read:(fun _ : (char, Bigarray.int8_unsigned_elt) bigarray -> assert false)
    ...
    let ba_opt_as_int32p =
      view
        ~read:(fun _ : (int32, Bigarray.int32_elt) bigarray option -> assert false)
        ~write:(function None -> null | Some b -> to_voidp (bigarray_start array1 b))
        (ptr void)
  ...
  module Glfw = struct
    ...
    let glproc : glproc typ = typedef (Foreign.funptr (void @-> returning void)) "GLFWglproc"

    (* 1391 *)
    type monitor

    let monitor : monitor structure typ = typedef (structure "GLFWmonitor") "GLFWmonitor"

    (* 1403 *)
    type window

    let window : window structure typ = typedef (structure "GLFWwindow") "GLFWwindow"

    (* 1590 *)
    type error_fun = int -> string -> unit

    let error_fun : error_fun typ =
      typedef (Foreign.funptr (int @-> string @-> returning void)) "GLFWerrorfun"

    (* 1635 *)
    type window_size_fun = window structure ptr -> int -> int -> unit

    let window_size_fun : window_size_fun typ =
      typedef (Foreign.funptr (ptr window @-> int @-> int @-> returning void)) "GLFWwindowsizefun"

    (* 1655 *)
    type window_close_fun = window structure ptr -> unit

    let window_close_fun : window_close_fun typ =
      typedef (Foreign.funptr (ptr window @-> returning void)) "GLFWwindowclosefun"

    (* 1696 *)
    type window_focus_fun = window structure ptr -> int -> unit

    let window_focus_fun : window_focus_fun typ =
      typedef (Foreign.funptr (ptr window @-> int @-> returning void)) "GLFWwindowfocusfun"
    ...
    let scroll_fun : scroll_fun typ =
      typedef (Foreign.funptr (ptr window @-> double @-> double @-> returning void)) "GLFWscrollfun"
    ...
    let key_fun : key_fun typ =
      typedef
        (Foreign.funptr (ptr window @-> int @-> int @-> int @-> int @-> returning void))
        "GLFWkeyfun"
  end
end
...
module Constants (T : Ctypes.TYPE) = struct
  module Gl = struct
    ...
    let active_attribute_max_length = constant "GL_ACTIVE_ATTRIBUTE_MAX_LENGTH" int
    ...
    let transform_feedback_buffer_mode = constant "GL_TRANSFORM_FEEDBACK_BUFFER_MODE" int

    (* 2123 *)
    let transform_feedback_varyings = constant "GL_TRANSFORM_FEEDBACK_VARYINGS" int
    ...
    (* 4863 *)
    let invalid_framebuffer_operation = constant "GL_INVALID_FRAMEBUFFER_OPERATION" int

    (* 5039 *)
    let program_binary_retrievable_hint = constant "GL_PROGRAM_BINARY_RETRIEVABLE_HINT" int

    let program_binary_length = constant "GL_PROGRAM_BINARY_LENGTH" int

    (* 5092 *)
    let geometry_shader_invocations = constant "GL_GEOMETRY_SHADER_INVOCATIONS" int

    (* 6312 *)
    let program_separable = constant "GL_PROGRAM_SEPARABLE" int

    (* 6482 *)
    let active_atomic_counter_buffers = constant "GL_ACTIVE_ATOMIC_COUNTER_BUFFERS" int

    (* 7086 *)
    let tess_control_output_vertices = constant "GL_TESS_CONTROL_OUTPUT_VERTICES" int
  ...
  module Glew = struct
    ...
    let error_glx_version_11_only = constant "GLEW_ERROR_GLX_VERSION_11_ONLY" int
...
module Bindings (F : Ctypes.FOREIGN) = struct
  module Glfw = struct
    ...
    (* 3284 *)
    let window_should_close = foreign "glfwWindowShouldClose" (ptr window @-> returning int)
    ...
    let set_window_user_pointer =
      foreign "glfwSetWindowUserPointer" (ptr window @-> ptr void @-> returning void)
    ...
    let set_cursor_pos_callback =
      foreign "glfwSetCursorPosCallback" (ptr window @-> cursor_pos_fun @-> returning cursor_pos_fun)

    (* 5417 *)
    let set_scroll_callback =
      foreign "glfwSetScrollCallback" (ptr window @-> scroll_fun @-> returning scroll_fun)
  ...
  module Gl = struct
    ...
    (* 1197 *)
    let viewport = foreign "glViewport" (int @-> int @-> int @-> int @-> returning void)

    (* 1717 *)
    let bind_buffer = foreign "glBindBuffer" (int_as_uint @-> int_as_uint @-> returning void)

    let buffer_data =
      foreign "glBufferData" (int_as_uint @-> int @-> ptr void @-> int_as_uint @-> returning void)

    let buffer_sub_data =
      foreign "glBufferSubData" (int_as_uint @-> int @-> int @-> ptr void @-> returning void)

    let delete_buffers = foreign "glDeleteBuffers" (int @-> ba_as_uint32p @-> returning void)

    (* 1723 *)
    let gen_buffers = foreign "glGenBuffers" (int @-> ba_as_int32p @-> returning void)

    (* 1924 *)
    let attach_shader = foreign "glAttachShader" (int_as_uint @-> int_as_uint @-> returning void)

    (* 1927 *)
    let compile_shader = foreign "glCompileShader" (int_as_uint @-> returning void)

    let create_program = foreign "glCreateProgram" (void @-> returning int_as_uint)

    let create_shader = foreign "glCreateShader" (int_as_uint @-> returning int_as_uint)

    let delete_program = foreign "glDeleteProgram" (int_as_uint @-> returning void)

    let delete_shader = foreign "glDeleteShader" (int_as_uint @-> returning void)

    let detach_shader = foreign "glDetachShader" (int_as_uint @-> int_as_uint @-> returning void)
    ...
    let get_program_info_log =
      foreign "glGetProgramInfoLog"
        (int_as_uint @-> int @-> ba_opt_as_int32p @-> ba_as_charp @-> returning void)

    let get_programiv =
      foreign "glGetProgramiv" (int_as_uint @-> int_as_uint @-> ba_as_int32p @-> returning void)
    ...
    let get_shaderiv =
      foreign "glGetShaderiv" (int_as_uint @-> int_as_uint @-> ba_as_int32p @-> returning void)
    ...
    let get_uniform_location =
      foreign "glGetUniformLocation" (int_as_uint @-> string @-> returning int)

    (* 1977 *)
    let uniform_matrix_4fv =
      foreign "glUniformMatrix4fv" (int @-> sizei @-> bool @-> ptr float @-> returning void)
    ...
    let gen_vertex_arrays = foreign "glGenVertexArrays" (int @-> ba_as_int32p @-> returning void)
  end
end
